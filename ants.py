...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant != None:
                ...
                #assert that one ant has to be able to contain the other
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            #there should be an exception here if insect == a bodyguarded imposter
            assert self.ant == insect, '{0} is not in {1}'.format(insect, self)

            if type(self.ant) is QueenAnt and not self.ant.imposter:
                return

            if type(self.ant) is BodyguardAnt:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive and current_place != None and current_range <= self.max_range:
            bee_list = current_place.bees
            bee = random_or_none(bee_list)

            if bee != None and current_range >= self.min_range:
                return bee
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        if self.digesting > 0:
            ...
            #eat_bee should eat a Bee, not None, so check if bee == None first.
            if bee != None:
                self.eat_bee(bee)
...
class QueenPlace(object):
    """An instance of this class is created when a QueenAnt object is created
    and replaces the Place object that is bound to AntColony.queen.
    It keeps track of the list of bees that are currently in the place of the 
    QueenAnt as well as the start of tunnel (the orignal place of the queen).
    """


    def __init__(self, home, qplace):
        self.home = home
        self.qplace = qplace

    @property
    def bees(self):
        return self.home.bees + self.qplace.bees
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        self.imposter = self.queenPlaced

        QueenAnt.queenPlaced = True
    ...
    def action(self, colony):
        ...
        #we only need to update colony.queen once
        if type(colony.queen) is not QueenPlace:
            colony.queen = QueenPlace(colony.queen, self.place)

        #QueenAnts throw
        ThrowerAnt.action(self, colony)

        #Double the damage of ants behind self
        ...
        while current_place != None:
            ...
            if ant != None:
                ...
                if ant.damage == type(ant).damage:
                    print('ant damage has been doubled')
                    ant.damage *= 2
